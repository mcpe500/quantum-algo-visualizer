...
import { buildVQEQuantumTabModel } from '../../engine/vqe/quantum-tab';
...
export function VQEQuantumTab({ result, circuitImage, trace }: VQEQuantumTabProps) {
  ...
  const {
    ...
    reset,
    jump,
  } = usePlaybackController(totalIterations, ANIMATION_INTERVAL_MS);

  const animatedIteration = playback.currentStep;
  const isAnimating = playback.isPlaying;

  const model = useMemo(() => {
    if (!result) return null;
    return buildVQEQuantumTabModel(result, activeCheckpoint, animatedIteration, isAnimating);
  }, [result, activeCheckpoint, animatedIteration, isAnimating]);
  ...
  const hasSnapshots = model.checkpoint.hasSnapshots;
  ...
  const handleCheckpointChange = (idx: number) => {
    ...
    const convIdx = hasSnapshots
      ? Math.min(
          result.quantum.iteration_snapshots[idx]?.iteration ?? idx * Math.ceil(totalIterations / result.quantum.iteration_snapshots.length),
          totalIterations - 1
        )
      : idx;
    jump(convIdx);
  ...
  return (
    <div className="space-y-6">
      <SectionCard title="Variational Quantum Eigensolver (VQE)" icon={<Cpu className="w-5 h-5" />}>
        <MetricsGrid>
          <MetricCard label="Qubits" value={model.metrics.qubits} />
          <MetricCard label="Iterations" value={model.metrics.iterations} />
          <MetricCard label="Circuit Depth" value={model.metrics.circuitDepth} />
          <MetricCard label="Gate Count" value={model.metrics.gateCount} />
        </MetricsGrid>
        ...
        <div className="flex flex-wrap gap-3 mb-6">
          ...
          <span className="px-3 py-1 bg-purple-100 text-purple-800 text-xs font-mono rounded-full">
            params: {model.parameters.optimalParameters.length}
          </span>
          ...
          <VQEMetricsGrid columns="comparison">
            <VQECard compact variant="success">
              ...
              <div className="text-xl font-mono text-emerald-900 mt-1">
                {model.comparison.fciEnergy.toFixed(6)} Ha
              </div>
            ...
            <VQECard compact variant="info">
              ...
              <div className="text-xl font-mono text-blue-900 mt-1">
                {model.comparison.vqeEnergy.toFixed(6)} Ha
              </div>
              <div className={VQE_TYPOGRAPHY.small + ' mt-1'}>
                Error: {model.comparison.energyError.toFixed(6)} Ha | {model.comparison.accuracyPercent.toFixed(2)}% accuracy
              </div>
            </VQECard>
          </VQEMetricsGrid>
          <div className="mt-4">
            <VQECard compact variant="warning">
              <p className="text-sm text-amber-800">
                <strong>Note:</strong> {model.comparison.note}
              </p>
            </VQECard>
          </div>
        </VQESection>
        ...
        {hasSnapshots && (
          <VQESection>
            <VQEStepFlowDiagram
              activeCheckpoint={model.checkpoint.safeActiveIndex}
          ...
          <VQESection>
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                ...
                <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider bg-${model.phaseBadge.tone}-100 text-${model.phaseBadge.tone}-700`}>
                  {model.phaseBadge.label}
                </span>
              </div>
            ...
            <ConvergenceChart
              data={model.convergence.history}
              optimalValue={model.convergence.optimalValue}
              title="VQE Convergence (Energy vs Iteration)"
              yLabel="Energy (Ha)"
              optimalLabel="FCI (exact)"
              dataLabel="VQE energy"
              animatedUpTo={model.convergence.animatedUpTo}
            />
          </VQESection>
        )}
        ...
        {hasSnapshots && (
          <VQESection>
            <VQECheckpointTimeline
              ...
              active={model.checkpoint.safeActiveIndex}
        ...
        {hasSnapshots && model.checkpoint.currentSnapshot && (
          <VQESection>
            <VQEHybridSplitView
              snapshot={model.checkpoint.currentSnapshot}
          ...
          <VQEMetricsGrid columns="energy">
            <VQECard compact variant="info">
              <div className={VQE_TYPOGRAPHY.caption}>
                {hasSnapshots ? `VQE Energy (Itr ${model.checkpoint.currentSnapshot?.iteration})` : 'VQE Energy'}
              </div>
              <div className="text-2xl font-mono font-bold text-blue-900 mt-1">
                {model.energy.displayEnergy.toFixed(6)} Ha
              </div>
            </VQECard>
        ...
        {/* Optimal Parameters */}
        {model.parameters.optimalParameters.length > 0 && (
          <VQESection>
            <h3 className="text-sm font-medium text-gray-700 mb-2">
              Optimized Parameters (θ₀ … θ{model.parameters.optimalParameters.length - 1})
            </h3>
            <div className="flex flex-wrap gap-2">
              {model.parameters.optimalParameters.map((p, i) => (
